...
HashPool是一个自定义的集合类型，使用散列表存储值，拥有很好的Add、Remove和Contain性能。

HashPool相对于HashSet更精简，并添加Get/GetToList等快速取值的方法，相比HashSet需要直接遍历，这样消耗更小。
...
// 两种同时获取多个物体的方法都有包含actionAfterGet参数的重载，可以选择为取出的每个物体执行想要的操作
...
在特定的场景（比如可以拖动进度条的场景）你可能需要在一帧内管理大量物体（比如1000个）并重新排列它们到正确的位置。使用UniPool的回收与生成功能，可以以很小的代价代替大量物体的管理与分配，使你的代码更为简洁高效。
